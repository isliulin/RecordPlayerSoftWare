...
int WAACDecoder::_OpenFile(unsigned int fSkipExtraChecking)
{
	...
	while(pos <= end)
	{
		...
		if(frames == 0)
        {
			...
			//yxhua202 修改于2015年12月17日；适应文件开头非AAC帧头的情况,获取第一帧后：
			c_pchPosition+=nStartPos;
			c_nSize-=nStartPos;
		...
		if (frame_length==0)
		{
           frame_length+=14;//对于静音数据，一般是14个数据；对于系统中间断电重启的情况会导致数据有零出现
		   pos += frame_length;
		}
		else
		{
    ...
    //原来是c_isInfo.nSampleRate = samplerate;
...
int WAACDecoder::Seek(unsigned int nSamples)
{
	...
	//原来是nSamples >=c_isInfo.nLength；当拖动刚好等于时，会报错。
