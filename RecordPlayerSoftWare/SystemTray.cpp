                         ...
                         HICON icon, UINT uID)
{
	...
	m_menu.LoadMenu(m_tnd.uID);	//ÅË°ËÈý, Modify at 2008-07-15
    // Set the tray icon
...
BOOL CSystemTray::Animate(UINT nDelayMilliSeconds, short nNumSeconds /*=-1*/)
{
    StopAnimation();
...
*/  CMenu *pSubMenu;	//ÅË°ËÈý, Modify at 2008-07-15
	
    if (m_menu.m_hMenu == NULL) return FALSE;
...
//       if (!menu.LoadMenu(m_tnd.uID)) return 0;
        if (m_menu.m_hMenu == NULL) return 0;	//ÅË°ËÈý, Modify at 2008-07-15
...
BOOL CSystemTray::SetMenu(HMENU hMenu)
{
	m_menu.Detach();
	m_menu.Attach(hMenu);
	...
	if (m_menu.m_hMenu != NULL)
	{
		...
		if (pSubMenu == NULL) return FALSE;
		int nCount = pSubMenu->GetMenuItemCount();
		CString strItem;
		for (int i = 0; i < nCount; i++)
			pSubMenu->GetMenuString(i, strItem,MF_BYPOSITION);
		return TRUE;
	}
	else
		return FALSE;
